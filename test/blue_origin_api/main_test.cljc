(ns blue-origin-api.main-test
  "Contract + behavioral test for the blue-origin-api L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "blue_origin_api-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'blue-origin-api.main-test)]
