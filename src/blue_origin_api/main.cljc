(ns blue-origin-api.main
  "Kotodama WASM entrypoint for the Blue_origin_api clean-room actor (L4) — Clojure port.
  ...
  facts (`blue_origin_api.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "blue_origin_api")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"blue_origin_api.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "blue_origin_api-compat" :tier tier :entities entities} 200])
...
(defn start! [] :blue_origin_api-compat/ready)
